...
- **키워드**: 그 글에 배정된 키워드를 본문에 자연스럽게 4~8회. 억지 반복(스터핑) 금지.
  키워드는 scripts/refill-queue.mjs 의 KEYWORD 풀에서 **지역·세부분야별로 돌려** 배정한다
  (전주/익산 × 형사·성범죄·이혼·민사). 분야 하나에 키워드 하나로 고정하면 159편을 써도
  노리는 검색어가 셋뿐이라 웹사이트 영역에서 자리가 안 난다.
- **지역은 본문으로 증명한다**: 익산 키워드면 관할(전주지방법원 군산지원·전주지방검찰청 군산지청)과
  이동·일정까지 구체적으로. 제목만 익산이고 본문이 전주 일반론이면 대량생성으로 걸러진다.
- **"전문변호사" 표기**: 법무법인 태앤규(김기태 변호사)는 대한변호사협회에
  **형사·노동법·이혼·부동산** 네 분야로 전문 등록되어 있다(2026-09-19 확인).
  이 넷은 제목·본문에 "○○전문변호사" 로 쓸 수 있다.
  등록되지 않은 분야(상속·교통사고·학교폭력 등)에는 절대 붙이지 않는다.
- **노동 사건**: 분야는 civil 로 분류하되 내용은 노동이다. 절차를 정확히 구분한다.
  임금·퇴직금 청구는 민사소송(법원), 부당해고 구제는 지방노동위원회(해고일로부터 3개월),
  산업재해는 근로복지공단이다. 이걸 섞어 쓰면 글 전체가 신뢰를 잃는다.
...
하루 3편을 발행하되 **한 세션이 1편씩만** 맡는다(예약 세션 3개, KST 07:47 / 08:47 / 09:47).
따라서 아래 기준은 "한 편을 실무자가 직접 쓴 최고 품질로 쓴다"는 전제이며, 이전 기준보다 우선한다.
